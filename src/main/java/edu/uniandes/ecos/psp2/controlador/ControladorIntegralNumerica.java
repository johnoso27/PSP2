...
import edu.uniandes.ecos.psp2.modelo.CalculadorIntegralNumerica;
import edu.uniandes.ecos.psp2.vista.VistaConsola;
import edu.uniandes.ecos.psp2.vista.VistaWeb;
...
public class ControladorIntegralNumerica extends HttpServlet {
    ...
    private static void calculoTest(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        try {
            ...
            double respuesta = calculador.calcularIntegralNumericaEnMargenError(10, 1.1, 0.000001, 9.0);
            VistaConsola.mostrarResultado("Datos de prueba:\n" + respuesta);
            VistaWeb.mostrarResultados(req, resp, respuesta, 10, 1.1, 0.000001, 9.0, "Integral numérica de datos de prueba");
        } catch (Exception ex) {
    ...
    private static void calculoEntrada(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try {
            ...
            CalculadorIntegralNumerica calculador = new CalculadorIntegralNumerica();
            double respuesta = calculador.calcularIntegralNumericaEnMargenError(segmento, x, e, dof);
            VistaConsola.mostrarResultado("Datos calculados de datos entrados:\n" + respuesta);
            VistaWeb.mostrarResultados(req, resp, respuesta, segmento, x, e, dof, "Integral numérica de los datos entrados");
        } catch (Exception ex) {
            VistaWeb.error(req, resp, ex);
        }
    }
